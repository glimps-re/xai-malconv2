...
from malconv import checkpoint  # This checkpoint implementation is faster than PyTorch's when using multiple GPUs
from malconv.inference.LowMemConvInf import LowMemConvBase
from malconv.inference.MalConvML_Inf import MalConvML
...
class MalConvGCT(LowMemConvBase):
    def __init__(
        ...
        self.convs = nn.ModuleList(
            ...
            + [nn.Conv1d(channels, channels * 2, window_size, stride=1, bias=True) for i in range(layers - 1)]
        )
        ...
        # self.custom_biais = nn.Parameter(torch.zeros(5, 2), requires_grad=True)
    ...
    def forward(self, x):
        ...
        post_conv, indices = self.seq2fix(x, pr_args={"gct": global_context[0]})
        # x = F.leaky_relu(self.fc_1( post_conv ))
        # x = self.fc_2(x)

        return post_conv, indices


class Extracted_MLP(torch.nn.Module):
    def __init__(self, channels=256, out_size=2):
        super(Extracted_MLP, self).__init__()
        self.fc_1 = torch.nn.Linear(channels, channels)
        self.fc_2 = torch.nn.Linear(channels, out_size)

    def forward(self, x):
        x = torch.nn.functional.leaky_relu(self.fc_1(x))
        x = self.fc_2(x)
        x = torch.nn.functional.softmax(x, dim=1)[:, 1]
        x = x.unsqueeze(1)
        return x
