...
class LowMemConvBase(nn.Module):
    ...
    def seq2fix(self, x, pr_args={}):
        ...
        if x.size(dim=1) < receptive_window:  # This is a tiny input! pad it out please
        ...
        with torch.no_grad():
            while start < end and (end - start) >= max(self.min_chunk_size, receptive_window):
                ...
                activ_win, activ_indx = F.max_pool1d(activs, kernel_size=int(activs.size(2)), return_indices=True)
        ...
        if len(chunk_list) > 1:
            ...
            x_selected = chunk_list[0].unsqueeze(0)
        ...
        return x_selected, winner_indices
